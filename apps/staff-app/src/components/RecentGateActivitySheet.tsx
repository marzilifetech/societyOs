...
/**
 * Bottom-sheet list of recent gate entries — opens from the
 * RecentGateActivityCard on the staff Home. Mirrors the resident-app sheet
 * but with staff styling + routes taps to /visitors/[id] for guard view.
 */
export function RecentGateActivitySheet({
  visible,
  onClose,
  visitors,
}: {
      ...
      <View style={styles.sheet}>
        ...
        <SafeAreaView edges={['bottom']}>
          <View style={styles.header}>
            <View style={{ flex: 1 }}>
              <Text style={styles.title}>Recent gate activity</Text>
              <Text style={styles.subtitle}>
                {visitors.length} {visitors.length === 1 ? 'entry' : 'entries'} in the last 24 hours
              </Text>
            </View>
          ...
          {visitors.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={styles.emptySub}>Approved arrivals will appear here.</Text>
            </View>
          ) : (
            <FlatList
              data={visitors}
              ...
              renderItem={({ item }) => (
                <Row
                  ...
                  onPress={() => {
                    onClose();
                    router.push(`/visitors/${item.id}` as any);
                  }}
                />
              )}
...
const styles = StyleSheet.create({
  ...
  row: {
    ...
    paddingVertical: 12, paddingHorizontal: 4, minHeight: 68,
  },
  avatar: {
